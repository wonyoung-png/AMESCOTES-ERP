...
export async function fetchVendors() {
  ...
  return (data || []).map((row: any) => ({
    ...
    processingUnitCost: row.processing_unit_cost ?? undefined,
    billingType: row.billing_type,
    settlementCycle: row.settlement_cycle,
    commissionRate: row.commission_rate ?? undefined,
    ttCondition: row.tt_condition,
    customMaterialType: row.custom_material_type,
...
export async function upsertVendor(vendor: Record<string, any>) {
  const row = filterForTable('vendors', {
    ...
    biz_reg_no: vendor.bizRegNo,
    address: vendor.address,
    billing_email: vendor.billingEmail,
    country: vendor.country,
    currency: vendor.currency,
    wechat_id: vendor.wechatId,
    lead_time_days: vendor.leadTimeDays,
    processing_unit_cost: vendor.processingUnitCost,
    billing_type: vendor.billingType,
    settlement_cycle: vendor.settlementCycle,
    commission_rate: vendor.commissionRate,
    tt_condition: vendor.ttCondition,
...
export async function fetchOrders() {
  ...
  return (data || []).map((row: any) => ({
    ...
    poBatchNo: row.po_batch_no || undefined,
    shipTo: row.ship_to || undefined,
    signatures: row.signatures || undefined,
    workOrderHtml: row.work_order_html || undefined,
    sentAt: row.sent_at || undefined,
    confirmedAt: row.confirmed_at || undefined,
    confirmedDate: row.confirmed_date || undefined,
    confirmNote: row.confirm_note || undefined,
...
export async function upsertOrder(order: Record<string, any>) {
  const row = filterForTable('production_orders', {
    ...
    po_batch_no: order.poBatchNo,
    ship_to: order.shipTo || null,
    signatures: order.signatures ?? {},
    work_order_html: order.workOrderHtml ?? null,
    sent_at: order.sentAt || null,
    confirmed_at: order.confirmedAt || null,
    confirmed_date: order.confirmedDate || null,
    confirm_note: order.confirmNote || null,
    ...
    post_defects: order.postDefects,
