...
export type OrderStatus = '초안' | '발주생성' | '생산중' | '생산완료' | '입고완료';
...
export const store = {
  ...
  saveBom: (bom: any): void => {
    ...
    const snakeBom: Record<string, any> = {
      ...
      // exchange_rate_usd 는 사전원가 USD 환율 자리다 — 사전만 바꾼 경우 덮이지 않게 한다
    ...
    try {
      ...
      for (const row of rows) {
        ...
        const converted = {
          ...
          exchangeRateUsd: row.post_exchange_rate_usd || row.exchange_rate_usd || 1380,
