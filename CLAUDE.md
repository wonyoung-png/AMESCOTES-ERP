...
### 🔴 배포 전 반드시 push (2026-08-10 사고 기록)

**빌드해서 배포만 하고 push를 안 하면, 다음 사람이 그 작업을 통째로 덮어쓴다.**

실제로 일어난 일: PMS(atlm-daily-check)에서 한쪽이 사이드바 개편·erp_link·erp_bridge를
만들어 배포만 하고 push하지 않았다. 다른 쪽이 자기 소스로 배포하자 그 작업이 서버에서
사라졌다. git에 없어서 복구할 곳도 없었다 — 컨테이너 `/app`을 꺼내 되살렸다.

규칙:
1. **배포 전에 commit + push를 먼저 한다.** 순서를 바꾸지 않는다.
2. 배포는 항상 git에서 clone/pull한 소스로 한다. 로컬 미커밋 상태로 배포 금지.
3. 남의 작업을 덮을 것 같으면 배포를 멈추고 먼저 확인한다.
   확인법: `docker exec <container> ls /app/routes /app/services` 로 서버에 있는데
   내 레포에 없는 파일이 있는지 본다.
4. 새 npm 의존성을 쓰면 `package.json`에도 반드시 넣는다.
   (lucide-react가 이미지에만 있어 재빌드가 안 됐다)

### 🔴 서버 트리를 통째로 커밋하지 말 것

복구한다고 `git add -A` 하면 `.env`·DB 덤프·zip이 딸려 들어간다.
실제로 실키 35개가 든 `.env.handover`가 원격에 올라가 키를 전부 회전해야 했다.
**커밋 전에 `git status --short`로 무엇이 담기는지 눈으로 확인한다.**
...
## 🔴 코덱스 최종확인 필수 (2026-09-14 대표 지시)

**모든 ERP 수정은 적용 전 코덱스 확인을 거친다.** 예외 없음.

1. 수정 후 `git diff > codex_check.diff`
2. 코덱스에 diff + 구체적 질문을 넘겨 리뷰
3. 지적 반영
4. 그 다음에 커밋·푸시

코덱스가 없거나 실패하면 **그 사실을 대표에게 그대로 보고**한다.
내가 대신 본 것을 코덱스 결과인 것처럼 말하지 않는다.
보고에는 **고객 화면에 보이는 변화가 있는지**를 항상 명시한다.
